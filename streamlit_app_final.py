...
def plot_anomaly_type_distribution(anomaly_type_distribution_df):
    ...
    try:
        ...
        # Sort by count
        anomaly_type_distribution = anomaly_type_distribution_df.sort_values("count", ascending=False)
...
@st.cache_data
def plot_risk_by_emirate(risk_scored_invoices_df, emirate_distribution_df):
    """Plot the average risk score by emirate with error handling"""
    try:
        if risk_scored_invoices_df is None or emirate_distribution_df is None or len(risk_scored_invoices_df) == 0 or len(emirate_distribution_df) == 0:
            # Return empty figure with message
            fig = go.Figure()
            fig.add_annotation(
        ...
        if "emirate" not in emirate_distribution_df.columns or "count" not in emirate_distribution_df.columns:
            # Return empty figure with message
            fig = go.Figure()
            fig.add_annotation(
        ...
        risk_by_emirate = pd.merge(risk_by_emirate, emirate_distribution_df, on="emirate", how="inner")
...
def main():
    ...
    # Get feature names for feature importance plot
    feature_names = feature_metadata.get("valid_features", []) if feature_metadata else []
    
    # Get confusion matrix for plotting
    confusion_matrix_data = evaluation_metrics.get("confusion_matrix") if evaluation_metrics else None
    ...
    with tab1:
        ...
        with col1:
            st.plotly_chart(plot_confusion_matrix(confusion_matrix_data), use_container_width=True)
        
        with col2:
            st.plotly_chart(plot_risk_score_distribution(risk_scored_invoices), use_container_width=True)
        
        # Feature importance - using extracted feature importances and names
        if feature_importances is not None and feature_names:
            st.plotly_chart(plot_feature_importance(feature_importances, feature_names), use_container_width=True)
        else:
            st.error("Feature importance data is not available.")
